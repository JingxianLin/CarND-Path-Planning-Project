...
int NextWaypoint(double x, double y, double theta, vector<double> maps_x, vector<double> maps_y, vector<double> maps_dx, vector<double> maps_dy)
{
  ...
  //If the inner product of v and h is positive then we are behind the waypoint so we do not need to increment closestWaypoint, 
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            for(int i = 0; i < sensor_fusion.size(); i++)
            {
              ...
              if(((vehicle_s-car_s) > -20) && ((vehicle_s-car_s) < 30) )
              {
                if(vehicle_d < 4)
                {
                  lane_0.push_back(vehicle_s);
                }
                else if(vehicle_d < 8)
                {
                  lane_1.push_back(vehicle_s);
                }
                else
                {
                  lane_2.push_back(vehicle_s);
                }
              }
            }
            ...
            // Create a list of widely spaced (x,y)
            // Later we will interpolate these waypoints
            vector<double> ptsx;
            vector<double> ptsy;
            ...
            else
            {
              ...
              //Redefine reference state as the previous path's end point
